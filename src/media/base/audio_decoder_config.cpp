...
void AudioDecoderConfig::Initialize(AudioCodec audio_codec,
  ...
  SampleFormat AudioDecoderConfig::sample_format(){
    return sample_format_;
  }

  int AudioDecoderConfig::sample_per_second(){
    return sample_per_second_;
  }

  ChannelLayout AudioDecoderConfig::channel_layout(){
  ...
  int AudioDecoderConfig::extra_data_size(){
    return extra_data_.size();
  }

  int AudioDecoderConfig::bytes_per_channel(){
    return bytes_per_channel_;
  }

  int AudioDecoderConfig::bytes_per_frame(){
  ...
  const char* AudioDecoderConfig::SampleFormatToString(SampleFormat sample_format){
  ...
  const char* AudioDecoderConfig::ChannelLayoutToString(ChannelLayout channel_layout){
  ...
  void AudioDecoderConfig::ShowAudioConfigInfo(){
    printf("audio:\n");
    printf("    codec:%s, sampleformat:%s, channellayout:%s, samplePerSecond:%d\n"
        , AudioCodecToString(audio_codec_)
        , SampleFormatToString(sample_format_)
        , ChannelLayoutToString(channel_layout_)
        , sample_per_second_);
  }
}  // namespace media