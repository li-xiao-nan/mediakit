...
PlaybackClock::PlaybackClock()
    : is_ticking_(false), playback_rate_(1.0), current_media_time_(0), media_play_time_(0) {
}
...
void PlaybackClock::SetPlaybackRate(float rate) {
  media_play_time_ += static_cast<int64_t>(wall_clock_timer_.Elapsed() * playback_rate_);
  playback_rate_ = rate;
...
void PlaybackClock::SetStartTime(int64_t start_time) {
  if (is_ticking_)
    return;
  media_play_time_ = start_time;
}

int64_t PlaybackClock::GetCurrentMediaTime() {
  current_media_time_ = media_play_time_ + static_cast<int64_t>(
      wall_clock_timer_.Elapsed() * playback_rate_);
